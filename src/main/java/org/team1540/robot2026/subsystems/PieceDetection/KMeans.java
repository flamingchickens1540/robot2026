...
public class KMeans {
    ...
    public int getLargestClusterIndex() {
    ...
    public static ArrayList<double[]> getPointsForCentroid(int[] assignment, double[][] allPoints, int centroidIndex) {
        ArrayList<double[]> pointsInCluster = new ArrayList<>();

        for (int i = 0; i < assignment.length; i++) {
            // Check if the current point's assignment matches the target centroid index
            if (assignment[i] == centroidIndex) {
                pointsInCluster.add(allPoints[i]);
            }
        }
