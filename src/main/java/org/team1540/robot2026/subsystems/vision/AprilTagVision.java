    ...
    public void periodic() {
        ...
        for (int i = 0; i < visionIOs.length; i++) {
            ...
            for (PoseObservation observation : cameraInputs[i].poseObservations) {
                ...
                for (AprilVisionIO.SingleTagObservation observationx : cameraInputs[i].singleTagObservations) {
                    robotState.addSingleTagMeasurement(observationx);
                }

                lastSeenTagPoses.addAll(Arrays.stream(cameraInputs[i].seenTagIDs)
                        .mapToObj(tagID ->
                                FieldConstants.AprilTagLayoutType.OFFICIAL.getLayout().getTagPose(tagID).orElse(Pose3d.kZero))
                        .toList());
            }
        }
